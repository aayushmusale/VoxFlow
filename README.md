...
2. **ASR Hallucinations:** Speech-to-text models (like Whisper) notoriously "hallucinate" words when exposed to background noise or silence.
...
## 🌍 Supported Languages
VoxFlow features dynamic language routing, supporting real-time, bi-directional translation across **10 Indic and Global languages**:
* **English, Hindi, Marathi, Bengali, Telugu, Tamil, Gujarati, Kannada, Malayalam, Punjabi.**
...
```mermaid
graph TD
    A[Microphone Input] -->|Hardware VAD Loop| B(Producer Thread: Audio Chunks)
    B -->|Thread-Safe Queue| C{Consumer Thread: Async Event Loop}
    C -->|Float16 CUDA Inference| D[Faster-Whisper: ASR]
    D -->|Network Executor| E[Google Neural API: Translation]
    E -->|Network Executor| F[Edge-TTS: Neural Voice Gen]
    F -->|Pygame Asynchronous Mix| G[Virtual Audio Cable / Output]
...
* **Hardware Filter:** An `800` volume threshold ignores low room static.
* **Duration Filter:** Ignores any sound lasting less than 0.5 seconds (blocks keyboard clacks and desk bumps).
* **Dynamic Language Lock:** Dynamically forces Whisper to only search for the user's selected input language, ignoring foreign static.
...
```bash
git clone [https://github.com/aayushmusale/VoxFlow.git](https://github.com/aayushmusale/VoxFlow.git)
cd VoxFlow
...
1. **Configure Windows Audio:** Ensure your physical hardware microphone is set as your Default Windows Input, and your physical speakers are set as your Default Windows Output.
2. Launch the application.
3. Select your **Source Language** (the language you are speaking) from the left dropdown.
4. Select your **Target Language** (the language your audience needs to hear) from the right dropdown.
5. Click **Start VoxFlow**.
6. Speak naturally into your microphone. The system will automatically detect when you finish a sentence, transcribe it, translate it, and play the regional neural voice aloud.
7. Click **Stop VoxFlow** to safely spin down the background threads and close the pipeline.
